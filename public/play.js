...
function loadGame(lessonId) {
    ...
    // Update iframe src to use proxy
    document.getElementById('lessonFrame').src = `/proxy/lessons/${lesson.lesson}`;
...
function applyFilters() {
    ...
    if (filteredGames.length === 0) {
        ...
        messageDiv.innerHTML = '<div class="alert alert-info">No games found matching your criteria.</div>';
...
function renderGames() {
    ...
    lessonsToShow.forEach(lesson => {
        ...
        // Set image using proxy
        const img = lessonItem.querySelector('img');
        img.src = `/proxy/lessons-img/${lesson.lesson}.webp`;
        img.alt = lesson.name;
